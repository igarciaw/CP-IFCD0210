...
function productSelection() {
  ...
  while (!products.includes(productSelected)) {
    console.warn(`"${productNumber}" no es una opción válida.`);
    userSelection();
  }
...
function insertCoins() {
  while (inserting) {
    ...
    while (!machineCoins.includes(coin)) {
      console.warn(`"${coin}" no es una opción válida.`);
      insertCoin();
    }
...
//// Máquina entrega de producto y devolución monedas
function tryAgain() {
  ...
  while (!validOptions.includes(userInput)) {
    console.warn(`"${userInput}" no es una opción válida.`);
    returnCoins();
  }
